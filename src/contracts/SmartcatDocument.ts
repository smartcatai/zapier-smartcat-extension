...
export function checkValidationStatus(documents: SmartcatDocument[], status:string): boolean {
    if (status == SmartcatDocumentValidationStatus.inProgress){
        let any = documents.find(value => value.documentDisassemblingStatus == SmartcatDocumentValidationStatus.inProgress);
        return !!any;
    }
    if (status == SmartcatDocumentValidationStatus.success){
        let all = documents.filter(value => value.documentDisassemblingStatus == SmartcatDocumentValidationStatus.success);
        return all.length == documents.length;
    }
    if (status == SmartcatDocumentValidationStatus.error){
        let any = documents.find(value => value.documentDisassemblingStatus == SmartcatDocumentValidationStatus.error);
        return !!any;
    }
...
export async function updateFile(z:ZObject, bundle:Bundle<{document:string, name:string, file:string}>): Promise<any> {
    const form = await GetFile(z, bundle.inputData.file, bundle.inputData.name);
...
export async function exportDocument(z:ZObject, bundle:Bundle<ExportModel>): Promise<{file:string, exportId: string}> {
    let queryParams = '?';
...
export async function getDocuments(z:ZObject, bundle:Bundle<{status: string | null, client: string | null, project:string | null}>): Promise<SmartcatDocument[]> {
    let projects = [];
    ...
    let documents = projects.map(value => value.documents).reduce((previousValue, currentValue) => previousValue.concat(currentValue));
    if (bundle.inputData.status) documents = documents.filter(value => value.status == bundle.inputData.status);
    return documents;
}

export async function searchDocuments(z:ZObject, bundle:Bundle<{name: string | null, client: string | null}>): Promise<SmartcatDocument[]> {
    let projects  = await getProjectList(z, bundle.authData.server, bundle.inputData.client);
    let documents = projects.map(value => value.documents).reduce((previousValue, currentValue) => previousValue.concat(currentValue));
    if (bundle.inputData.name) documents = documents.filter(value => value.name.includes(bundle.inputData.name as string));
    return documents;
}
