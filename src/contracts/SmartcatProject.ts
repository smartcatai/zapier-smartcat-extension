...
export async function getProjects(
    z: ZObject,
    bundle: Bundle<{ status: string | null; validation: string | null; client: string | null }>,
): Promise<SmartcatProject[]> {
    ...
    if (bundle.inputData.validation)
        projects = projects.filter(value =>
            checkValidationStatus(value.documents, bundle.inputData.validation as string),
        );
    return projects;
}
...
export async function searchProjects(
    ...
    if (bundle.inputData.name)
        projects = projects.filter(value => value.name.includes(bundle.inputData.name as string));
    return projects;
}
...
export async function createProject(z: ZObject, bundle: Bundle): Promise<SmartcatProject> {
    ...
    form.append('body', z.JSON.stringify(createCommand), { contentType: 'application/json' });
